...
The release configuration process involves creating various Custom Resources in OpenShift. Use the link:https://github.com/konflux-ci/konflux-ci/blob/release-0.1/operator/upstream-kustomizations/cli/setup-release.sh[setup-release.sh wrapper script] to simplify this process.

Log in to the {OCPShortName} cluster as the Platform Engineer (cluster admin) and run the script:

[source,bash]
----
curl -O https://raw.githubusercontent.com/konflux-ci/konflux-ci/refs/heads/release-0.1/operator/upstream-kustomizations/cli/setup-release.sh
