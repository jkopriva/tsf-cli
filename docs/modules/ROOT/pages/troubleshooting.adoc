...
Locate the Cert-Manager entry and change `manageSubscription` to `auto` or `false`. Then re-run the deployment. See xref:installing.adoc#_configure_the_cluster[Configure the cluster] for details on `manageSubscription` values.
